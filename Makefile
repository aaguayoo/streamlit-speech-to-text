########################################################################################
##########  MAKEFILE FOR STREAMLIT-SPEECH-TO-TEXT  ##########
########################################################################################
...
ifdef version
	...
	@cat streamlit-speech-to-text/__init__.py | awk '/^__version__ =/{gsub($$3,"\"${version}\"")};{print}' > temp && mv temp streamlit-speech-to-text/__init__.py
	@cat tests/test_streamlit-speech-to-text.py | awk '/__version__ ==/{gsub($$4,"\"${version}\"")};{print}' > temp && mv temp tests/test_streamlit-speech-to-text.py
	@echo ""
	@echo "\033[0;35m##################\033[0m"
...
ifdef version
	...
	@poetry run python ${PROJECT_PATH}/profiling/streamlit-speech-to-text_profiling.py > docs/img/memory_profiler_v${version}.log
	...
	@poetry run python ${PROJECT_PATH}/profiling/streamlit-speech-to-text_profiling.py > profiling/memory_profiler.log && echo "\033[0;32mProfiling log saved in profiling/memory_profiler.log.\033[0m" || echo "\033[0;31mError running performance analysis.\033[0m"
...
help:
	@echo "#############################################################"
	@echo "##          MAKEFILE FOR STREAMLIT-SPEECH-TO-TEXT          ##"
	@echo "#############################################################"
	@echo ""
	@echo "   Targets:   "
	@echo ""
